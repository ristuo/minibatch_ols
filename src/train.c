...
void *produce(void *args) {
  ...
  dataset_ptr ds = dataset_from_io(io_res, target_var_column);
...
void train2(char *file_path, char separator, index target_var_column,
            index max_sweeps, index batch_size) {
  ...
  int cons_start =
      pthread_create(&consumer, NULL, consume, (void *)(&consumer_args));
  if (cons_start) {
...
void train(char *file_path, char separator, index target_var_column,
           int max_sweeps, index batch_size) {
  stop_cond_ptr stop = stop_cond_create(CONVERGENCE, 0.00000001);
  model_ptr model = model_create(stop);
  int i = 0;
  FILE *fp = fopen(file_path, "r");
  ...
  while (i < max_sweeps) {
    io_res_ptr input = read_batch_fp(fp, separator, batch_size, file_path);
